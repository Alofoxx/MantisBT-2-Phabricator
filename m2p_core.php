...
function checkbtUser(){
  ...
  if(isset($_POST['button']) && $_POST['button'] === "Save and Continue"){
    ...
    foreach ($_SESSION['phab']['users'] as $user) {
      ...
      if($_POST[$user['id'].'_enabled'] == true || $_POST[$user['id'].'_enabled'] == false){
...
function checkbtProject(){
  ...
  if(isset($_POST['button']) && $_POST['button'] === "Save and Continue"){
    ...
    foreach ($_SESSION['phab']['projects'] as $row) {
      ...
      //optional fields
      if($_POST[$row['id'].'_enabled'] == true || $_POST[$row['id'].'_enabled'] == false){
        //save Enabled if valid post is recieved.
  ...
  if(!isset($_SESSION['phab']['versions'])){
    ...
    foreach($_SESSION['phab']['versions'] as $project){
      foreach ($project as $row) {
        $_SESSION['phab']['versions'][$row['project_id']][$row['id']]['milestone'] = $row['released'];
      }
    }
  }
    ...
    foreach($_SESSION['phab']['versions'] as $project){
      ...
      foreach ($project as $row) {
        ...
        if(trim($_POST[$row['id'].'_version']) == "") {
          ...
          //save
          $_SESSION['phab']['versions'][$row['project_id']][$row['id']]['version'] = trim($_POST[$row['id'].'_version']);
        }

        //optional fields
        if($_POST[$row['id'].'_milestone'] == true || $_POST[$row['id'].'_milestone'] == false){
          //save milestone if valid post is recieved.
          $_SESSION['phab']['versions'][$row['project_id']][$row['id']]['milestone'] = $_POST[$row['id'].'_milestone'];
        }
      }
    }
    //if there is an error stop here.
